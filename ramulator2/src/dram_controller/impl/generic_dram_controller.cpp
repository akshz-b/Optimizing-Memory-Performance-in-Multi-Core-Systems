...
namespace Ramulator
{
  ...
  public:
    ...
    bool send(Request &req) override
    {
      ...
      if (req.type_id == Request::Type::Read)
      {
        ...
        if (!is_success)
        {
          // std::cerr << "Request dropped due to full  read buffer!\n";
          s_num_read_reqs--;
        }
      }
      else if (req.type_id == Request::Type::Write)
      {
        ...
        if (!is_success)
        {
          // std::cerr << "Request dropped due to full  write buffer!\n";
          s_num_write_reqs--;
        }
      }
      else
      ...
      if (!is_success)
      {
        ...
        req.arrive = -1;
        // std::cerr << "Request dropped due to full buffer!\n";
        return false;
      }
  ...
  private:
      ...
      if (pending.size())
      {
        ...
        if (req.depart <= m_clk)
        {
          ...
          if (req.depart - req.arrive > 1)
          {
            ...
            // if (req.depart - req.arrive > 10000)
            // {
            //   std::cout << "Request " << req.addr << " took " << req.depart - req.arrive << " cycles to complete.\n";
            // }
            s_read_latency += 2*core_channel_latency_matrix[req.source_id][req.addr_vec[0]];
          }
    ...
    void finalize() override
    {
      ...
      overall_latency += s_read_latency;
      overall_request += s_num_read_reqs;
