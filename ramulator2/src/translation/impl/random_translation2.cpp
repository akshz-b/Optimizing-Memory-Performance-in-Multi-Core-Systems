...
namespace Ramulator
{
  ...
  class RandomTranslation2 : public ITranslation, public Implementation
  {
    RAMULATOR_REGISTER_IMPLEMENTATION(ITranslation, RandomTranslation2, "RandomTranslation2", "Randomly allocate physical pages to virtual pages.");
  ...
  protected:
    ...
    std::unordered_map<size_t, size_t> global_page_table;
  ...
  public:
    void init() override
    {
      ...
      m_logger = Logging::create_logger("RandomTranslation2");
    };
    ...
    bool translate(Request &req) override
    {
      ...
      auto target = global_page_table.find(vpn);

      if (target == global_page_table.end())
      {
        ...
        if (m_num_free_physical_pages == 0)
        {
          ...
          global_page_table[vpn] = ppn_to_replace;
        ...
        else
        {
          ...
          while (
              (m_reserved_pages.find(ppn_to_assign) != m_reserved_pages.end()) || (!m_free_physical_pages[ppn_to_assign]))
          {
            ppn_to_assign = m_allocator_rng() % m_num_pages;
          }
          global_page_table[vpn] = ppn_to_assign;
          
          m_num_free_physical_pages--;
          m_free_physical_pages[ppn_to_assign] = false;
        }
        ...
        // m_logger->info("Translated Addr {}, VPN {} to Addr {}, PPN {}.", req.addr, vpn, p_addr, global_page_table[vpn]);
      ...
      {
        // We found an existing translation
        Addr_t p_addr = (global_page_table[vpn] << m_offsetbits) | (req.addr & ((1 << m_offsetbits) - 1));
        DEBUG_LOG(DTRANSLATE, m_logger, "Translated Addr {}, VPN {} to Addr {}, PPN {}.", req.addr, vpn, p_addr, global_page_table[vpn]);
        // m_logger->info("Translated Addr {}, VPN {} to Addr {}, PPN {}.", req.addr, vpn, p_addr, page_table[vpn]);
        req.addr = p_addr;
        return true;
      }
